...
actionModel = quadruped_walkgen.ActionModelQuadrupedStepPeriod()
actionModel.dt_weight = 0.
actionModel.shoulderWeights =  np.zeros(8)
...
def run_calcDiff_numDiff(epsilon) :
  ...
  for k in range(N_trial):    
    ...
    u = a + (b-a)*np.random.rand(5)
    u[-1] = np.random.rand(1)[0] #dt > 0 
