...
# MPC optim period 1
mpc_planner_time = MPC_crocoddyl_planner(dt = dt_mpc , T_mpc = T_gait , n_periods = n_periods)
...
mpc_planner_time_2.relative_forces = False
mpc_planner_time_2.nb_nodes_horizon = 15
...
for i in range(iteration-1) : 
    ...
    interface.l_feet = l_feet_[:,:,i]

    mpc_planner_time_2.solve(i , xref[:,:,i]  , interface )
...
# Xs_1 , Us_1 , lt_state_1 , lt_force_1 , Cost_1 , x_dt_change_1 , x_foot_change_1 , results_dt_1 , fsteps_1 = get_results(ddp1, False)
...
gait = mpc_planner_time_2.gait
Xs_2 , Us_2 , lt_state_2 , lt_force_2 , Cost_2 , x_dt_change_2 , x_foot_change_2 , results_dt_2 , fsteps_2 = get_results(ddp2 , True)


Xs_1 = []
Us_1 = []
for index,elt in enumerate(ddp1.problem.runningModels) :
    if elt.nu != 4 : 
        Xs_1.append(ddp1.xs[index])
        Us_1.append(ddp1.us[index])

Xs_1.append(ddp1.xs[-1]) #terminal node
Xs_1 = np.array(Xs_1).transpose()
Us_1 = np.array(Us_1).transpose()

lt_state_1 = np.linspace(0,dt_mpc*(Xs_1.shape[1] - 1 ), Xs_1.shape[1]  )
lt_force_1 = np.linspace(0,dt_mpc*(Us_1.shape[1] -1), Us_1.shape[1]  )
#############
# Plot cost
#############
