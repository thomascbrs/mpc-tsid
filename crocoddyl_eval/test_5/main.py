...
def run_scenario(envID, velID, dt_mpc, k_mpc, t, n_periods, T_gait, N_SIMULATION_, type_MPC, pyb_feedback, desired_speed):
    ...
    joystick, fstep_planner, logger, interface = utils.init_objects(
        dt, dt_mpc, N_SIMULATION, k_mpc, n_periods, T_gait, True)
    ...
    for k in range(int(N_SIMULATION)):
        ...
        for i in range(4):
                ...
                pos_tmp = np.reshape(np.array(interface.oMl * (np.array([fstep_planner.fsteps[index, (1+i*3):(4+i*3)]]).transpose())) , (3,1) )
                o_feet_heur[:2, i , int(k/k_mpc)] = pos_tmp[0:2, 0]
                pos_tmp = np.reshape( np.array([fstep_planner.fsteps[index, (1+i*3):(4+i*3)]]).transpose() , (3,1) )
                pos_pred_local_heur[:2, i , int(k/k_mpc)] =  pos_tmp[0:2, 0]
    ...
    print("Vx = " , desired_speed[0] , "  ;  Vw = " , desired_speed[5] , "  -->  " , finished )


    return o_feet_ , o_feet_heur , gait_ , pred_trajectories ,l_feet_ , pos_pred_local , pos_pred_local_heur , logger
